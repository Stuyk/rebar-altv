...
export function useEvents() {
   ...
   *
   */
  function emitServer<EventNames = string>(
    eventName: EventNames,
    ...args: any[]
  ): void {
    if (!("alt" in window)) {
      console.log(
        `[Webview] To Server Mock: ${eventName} | Args: ${JSON.stringify(args)}`
      );
      return;
    }
   ...
   *
   */
  function emitClient<EventNames = string>(
    eventName: EventNames,
    ...args: any[]
  ): void {
    if (!("alt" in window)) {
      console.log(
        `[Webview] To Client Mock: ${eventName} | Args: ${JSON.stringify(args)}`
      );
      return;
    }
   ...
   *
   */
  function on<EventNames = string, Callback = (...args: any[]) => void>(
    eventName: EventNames,
    callback: Callback
  );
